...
export function AnalysisLoading({ onComplete }: AnalysisLoadingProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh]">
      ...
      <motion.div
        className="w-20 h-20 rounded-full bg-gradient-to-br from-gray-800 to-gray-900 dark:from-gray-100 dark:to-gray-200
      ...
      >
        <div className="flex gap-1">
          {[0, 1, 2].map((i) => (
            <motion.div
              key={i}
              className="w-2 h-2 bg-white dark:bg-gray-800 rounded-full"
              animate={{ y: [0, -8, 0] }}
              transition={{
                duration: 0.6,
                repeat: Infinity,
      ...
      <AnimatePresence mode="wait">
        <motion.p
          ...
          exit={{ opacity: 0, y: -10 }}
          className="text-xl font-medium text-gray-900 dark:text-gray-100 mb-6"
        >
      ...
      {/* Progress Bar */}
      <div className="w-80 h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
        <motion.div
        ...
        {stages.map((stage, i) => (
          <motion.div
            ...
            className={`w-2 h-2 rounded-full transition-colors duration-300
              ${i <= currentStage ? 'bg-gray-900 dark:bg-gray-100' : 'bg-gray-200 dark:bg-gray-700'}`}
            animate={i === currentStage ? { scale: [1, 1.3, 1] } : {}}
            transition={{ duration: 0.5, repeat: i === currentStage ? Infinity : 0 }}
          />
        ))}
      </div>
    </div>
